...
function FeedbackForm() {
  ...
  const handleFormSubmit = (e) => {
    ...
    axios
      .post('http://localhost:8080/auth/feedback/submit', feedbackData)
      .then((response) => {
      ...
      <form onSubmit={handleFormSubmit} className="feedback-form">
        ...
        <label className="feedback-label">
          ...
          <Rating
            name="custom-star-color"
            defaultValue={0}
            onChange={(event, newValue) => setComfort(newValue)}
            precision={1}
            IconContainerComponent={StarIcon}
            emptyIcon={<StarIcon style={{ color: 'grey' }} />} 
            icon={<StarIcon style={{ color: 'gold' }} />} 
        ...
        <label className="feedback-label">
          ...
          <Rating
            ...
            defaultValue={0}
            precision={1}
            onChange={(event, newValue) => setValueForMoney(newValue)}
            IconContainerComponent={StarIcon}
            emptyIcon={<StarIcon style={{ color: 'grey' }} />} 
            icon={<StarIcon style={{ color: 'gold' }} />} 
        />
